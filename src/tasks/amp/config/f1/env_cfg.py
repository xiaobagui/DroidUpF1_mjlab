"""F1 flat-ground velocity-commanded locomotion AMP task."""
...
from src.assets.f1 import F1_ACTION_SCALE, get_f1_robot_cfg
from src.tasks.amp import mdp
from src.tasks.amp.config.f1.constants import (
  F1_ACTOR_FRAME_DIM,
...
def f1_walk_amp_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Build the local F1 walk-and-run AMP task."""
  all_joints = SceneEntityCfg(
    "robot", joint_names=F1_JOINT_NAMES, preserve_order=True
  )
  ...
  key_bodies = SceneEntityCfg(
    "robot", body_names=F1_AMP_KEY_BODY_NAMES, preserve_order=True
  )
      ...
      terms={
        "frame": ObservationTermCfg(
          ...
          params={
            ...
            "actor_frame_dim": F1_ACTOR_FRAME_DIM,
            "critic_frame_dim": F1_CRITIC_FRAME_DIM,
          },
          clip=(-100.0, 100.0),
        )
      },
    ...
    "amp": ObservationGroupCfg(
      terms={
        "state": ObservationTermCfg(
          ...
          params={
            "joint_cfg": all_joints,
            "key_body_cfg": key_bodies,
            "amp_obs_dim": F1_AMP_OBS_DIM,
          },
          clip=(-100.0, 100.0),
        )
      },
  ...
  cfg.scene.entities = {
    "robot": get_f1_robot_cfg(action_delay_range=(0, 0) if play else (0, 2))
  }
  cfg.scene.sensors = (
    ContactSensorCfg(
